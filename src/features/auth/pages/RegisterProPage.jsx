    ...
    <div>
      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-2xl p-5 cursor-pointer transition-all group ${
          file ? 'border-emerald-400 bg-emerald-50/60' : 'border-slate-200 hover:border-orange-300 bg-slate-50/60 hover:bg-orange-50/20'
        }`}
      >
        ...
        {file ? (
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="text-sm font-semibold text-slate-600 group-hover:text-navy-900 transition-colors">Cliquez pour importer</div>
...
// ── Labeled field ─────────────────────────────────────────────────────────────
function WizField({ label, icon: Icon, value, set, placeholder, hint, type = 'text', required = false }) {
  return (
    <div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <WizField label="Nom de l'agence *" icon={I.Building} value={agencyName} set={setAgencyName} placeholder="Agence Immobilière du Lac" required />
        <div className="grid grid-cols-2 gap-4">
          <WizField label="SIRET *" icon={I.BadgeCheck} value={siret} set={setSiret} placeholder="12345678900012" hint="14 chiffres" required />
          <WizField label="Téléphone *" icon={I.Phone} value={phone} set={setPhone} placeholder="+33 6 12 34 56 78" required />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <WizField label="Ville *" icon={I.MapPin} value={city} set={setCity} placeholder="Paris" required />
          <WizField label="Site web" icon={I.Globe} value={website} set={setWebsite} placeholder="https://mon-agence.fr" />
        </div>
        <WizField label="Adresse professionnelle" icon={I.MapPin} value={address} set={setAddress} placeholder="12 rue du Commerce, 75011 Paris" />
      </div>
    </div>
  )
}
...
function WizStep2({ fullName, setFullName, email, setEmail, password, setPassword, confirmPwd, setConfirmPwd, show, setShow }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <WizField label="Nom complet du responsable *" icon={I.User} value={fullName} set={setFullName} placeholder="Jean Dupont" required />
        <WizField label="E-mail professionnel *" icon={I.Mail} value={email} set={setEmail} placeholder="direction@agence.fr" type="email" required />
        <div>
...
function WizStep3({ logo, setLogo, logoUrl, description, setDescription, linkedin, setLinkedin }) {
  return (
    <div className="space-y-6">
      ...
      <WizField label="Page LinkedIn" icon={I.Link} value={linkedin} set={setLinkedin} placeholder="https://linkedin.com/company/mon-agence" />
    </div>
  )
}
...
const PRO_PLANS = [
  { id: 'starter',    name: 'Starter',    monthly: 29,  yearly: 24,  highlight: false, badge: null,         features: ['5 annonces actives', 'Statistiques simples', 'Badge Pro', 'Support e-mail'] },
  { id: 'business',   name: 'Business',   monthly: 79,  yearly: 66,  highlight: true,  badge: 'Recommandé', features: ['Annonces illimitées', 'CRM leads intégré', 'Boost visibilité ×3', 'Analytics avancés', 'Support prioritaire'] },
  { id: 'enterprise', name: 'Enterprise', monthly: 199, yearly: 166, highlight: false, badge: null,         features: ['Multi-agents', 'Accès API complet', 'Support dédié', 'Import massif', 'SLA garanti'] },
]
  ...
  return (
    <div className="space-y-6">
          ...
          return (
            <motion.button key={p.id} type="button" onClick={() => setPlan(p.id)}
              ...
              className={`relative text-left rounded-3xl border-2 overflow-hidden transition-all duration-200 ${
                p.highlight ? 'border-orange-500 bg-[#0B1F3A] shadow-2xl shadow-orange-500/25'
                : active     ? 'border-orange-500 bg-white shadow-lg shadow-orange-500/10'
                :              'border-slate-200 bg-white hover:border-slate-300 hover:shadow-md'
              }`}>
              {p.highlight && <div className="absolute -top-12 -right-12 w-40 h-40 rounded-full bg-orange-500 opacity-20 blur-3xl pointer-events-none" />}
              <div className="p-5 relative z-10">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {p.badge && (
                    <span className="bg-orange-500 text-white text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase tracking-wider">{p.badge}</span>
                  )}
                </div>
                <div className={`text-[11px] font-bold uppercase tracking-widest mb-1 ${p.highlight ? 'text-orange-400' : 'text-slate-500'}`}>{p.name}</div>
                ...
                {yearly && <div className={`text-[11px] mb-1 ${p.highlight ? 'text-slate-400' : 'text-slate-500'}`}>soit {price * 12}€/an</div>}
                  ...
                  {p.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-xs">
                      <span className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${p.highlight ? 'bg-orange-500/25' : 'bg-emerald-50'}`}>
                        <I.Check size={9} className={p.highlight ? 'text-orange-400' : 'text-emerald-600'} />
                      </span>
      ...
      <div className="flex items-center justify-center gap-2 text-xs text-slate-400">
        ...
        <span className="text-slate-300">·</span>Résiliable à tout moment
        <span className="text-slate-300">·</span>Pas d'engagement
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className={`relative overflow-hidden rounded-2xl border-2 ${selectedPlan.highlight ? 'border-orange-500 bg-[#0B1F3A]' : 'border-slate-200 bg-white'}`}>
        {selectedPlan.highlight && <div className="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-orange-500 opacity-15 blur-3xl pointer-events-none" />}
        <div className="relative z-10 px-5 py-4">
          <div className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${selectedPlan.highlight ? 'text-orange-400' : 'text-slate-400'}`}>Plan sélectionné</div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className={`font-extrabold text-base leading-tight ${selectedPlan.highlight ? 'text-white' : 'text-[#0F172A]'}`}>
                  {selectedPlan.name}
                  {selectedPlan.badge && (
                    <span className="ml-2 text-[10px] font-extrabold bg-orange-500 text-white px-2 py-0.5 rounded-full uppercase tracking-wider align-middle">{selectedPlan.badge}</span>
                  )}
                </div>
                <div className={`text-xs mt-0.5 ${selectedPlan.highlight ? 'text-slate-400' : 'text-slate-500'}`}>Facturation mensuelle · résiliable à tout moment</div>
              </div>
            </div>
            <div className="flex flex-col items-end gap-2 shrink-0">
              <div className="text-right">
                <span className={`text-2xl font-extrabold ${selectedPlan.highlight ? 'text-white' : 'text-[#0F172A]'}`}>{selectedPlan.monthly}€</span>
              ...
              <button type="button" onClick={onEditPlan}
                ...
                <I.ChevronLeft size={11} /> Modifier
              </button>
            </div>
          </div>
            ...
            {selectedPlan.features.map((f) => (
              <span key={f} className="flex items-center gap-1.5 text-xs">
                <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0 ${selectedPlan.highlight ? 'bg-orange-500/25' : 'bg-emerald-50'}`}>
                  <I.Check size={8} className={selectedPlan.highlight ? 'text-orange-400' : 'text-emerald-600'} />
                </span>
        ...
        <div className={`px-5 py-2.5 flex items-center gap-2 border-t ${selectedPlan.highlight ? 'border-white/10 bg-white/5' : 'border-slate-100 bg-slate-50'}`}>
          ...
          <span className="text-[11px] text-slate-400">Paiement sécurisé via <span className="font-bold text-slate-500">Stripe</span></span>
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
        ...
        <div className="relative inline-flex items-center justify-center mb-8">
          <motion.div
            ...
            <I.Check size={44} className="text-white" />
          </motion.div>
        ...
        <div className="space-y-3 mb-8">
          {NEXT_STEPS.map(({ Icon, label, delay }) => (
            <motion.div key={label}
              ...
              className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-5 py-3.5 text-left">
        ...
        <motion.button
          ...
          onClick={onHome}
          className="w-full h-12 rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-bold transition-colors">
          Retour à l'accueil
        </motion.button>
      </motion.div>
    </div>
...
export default function RegisterProPage() {
  ...
  if (submitted) {
    return <ProSuccessScreen email={email} agencyName={agencyName} plan={plan} onHome={() => navigate('/')} />
  }
                    ...
                    return (
                      <div key={s.n} className={`relative flex items-start gap-3 px-3 py-2.5 rounded-2xl transition-all duration-200 ${active ? 'bg-white shadow-sm border border-slate-100' : ''}`}>
                        ...
                        <div className="min-w-0 pt-0.5">
                          <div className={`text-xs font-bold leading-snug transition-colors ${active ? 'text-[#0F172A]' : done ? 'text-slate-500' : 'text-slate-300'}`}>{s.label}</div>
                          {(active || done) && <div className="text-[10px] text-slate-400 mt-0.5 leading-tight">{s.desc}</div>}
                        </div>
                      </div>
                    )
                  })}
                ...
                <div className="flex items-center gap-3">
                  ...
                  {step < 5 ? (
                    <motion.button type="button" onClick={() => goTo(step + 1)}
                      whileHover={{ scale: 1.025, boxShadow: '0 14px 36px rgba(234,88,12,0.32)' }} whileTap={{ scale: 0.97 }}
                    ...
                    <motion.button type="button" onClick={submit} disabled={loading}
                      whileHover={!loading ? { scale: 1.025, boxShadow: '0 14px 36px rgba(234,88,12,0.32)' } : {}} whileTap={{ scale: 0.97 }}
